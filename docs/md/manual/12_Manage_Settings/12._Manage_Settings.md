...
The "Export users" button allows administrator to export users. There are 2 export options: default configuration and custom configuration.  
...
1. Default configuration  
    ...
    To export default file user can press the "Export users" button or the arrow button at the right of the "Export users" and select "Default configuration" at the drop-down menu.

2. Custom configuration  
    Custom configuration allows user to select which user attributes should be downloaded. After pressing the "Custom configuration" button at the drop-down menu the system will display the modal window with the list of available attributes:  
        ![CP_v.0.16_ReleaseNotes](attachments/ManageSettings_13.png)  
    User can select any set of attributes by marking/unmarking corresponding checkboxes. At least 1 checkbox should be marked to export users.  
    To download the custom file press the "Download CSV" button. The Excel file with corresponding attributes will be downloaded to user's local machine.
